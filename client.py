...
"""Here I'm thinking we need 2 threads in addition to the main thread. One that constantly checks 
    a list to see if there are user msg's to be send over UDP and then same for TCP. When user 
    decides from main thread to answer at terminal they want to register they'll give some info and 
    we create a msg and put into the list the UDP thread keeps checking"""
udp_messages = []
...
CHOICES = ['r']
HOST = "192.168.0.106"  # this would normally be different and particular to the host machine ie client
PORT = 5001  # UDP port
# todo check w/ prof that the server ip address will be known and don't need to get it programmatically
SERVER = ("192.168.0.106", 5023)
